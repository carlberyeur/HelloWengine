...
namespace cu
{
	...
	public:
		...
		const Vector3<TYPE> GetEulerRotation() const
		{
			...
			return Vector3<TYPE>(aX, aY, aZ);
		}

		Matrix33 Lerp(const Matrix33& aInterpolateToMatrix, const TYPE aInterpolatingSpeed)
		{
		...
		Matrix33 LookAt(const Vector3<TYPE>& aPositionToLookFrom, const Vector3<TYPE>& aPositionToLookAt)
		{
			Vector3<TYPE> objectUpVector(0.0f, 1.0f, 0.0f);

			float xSize = myRightVector.Length();
			float ySize = myUpVector.Length();
			float zSize = myForwardVector.Length();

			Vector3<TYPE> zAxis = aPositionToLookAt - aPositionToLookFrom;
			if (zSize <= 0)
			{
			...
			Vector3<TYPE> xAxis = objectUpVector.Cross(zAxis);
			if(xSize <= 0)
			{
			...
			Vector3<TYPE> yAxis = zAxis.Cross(xAxis);
			if (ySize <= 0)
			{
...
#pragma warning( disable : 4201 ) // remove warning namless struct
		union
		{
			...
			struct
			{
				Vector2<TYPE> myRightVector2D;	TYPE ms13;
				Vector2<TYPE> myUpVector2D;		TYPE ms23;
				Vector2<TYPE> myPosition2D;		TYPE ms33;
			};
